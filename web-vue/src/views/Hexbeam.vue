...
// Faktoren aus den 20m-Referenzmaßen der G3TXQ-Bauanleitung:
//   ½ Driver = 214"  = 5.44 m  →  0.2570 × λ   (Drahtlänge, 3D entlang Spreader-Bogen)
//   Reflector = 404" = 10.26 m →  0.4849 × λ   (Drahtlänge, Gesamtdraht)
//   Tip Spacer = 24" = 0.61 m  →  0.0288 × λ   (PVC-Isolator)
//   Spreader-Radius (horiz.) ≈ 3.46 m für 20m  →  0.1635 × λ
function bandData(b) {
  const lambda = 300 / b.fMHz
  return {
    ...b,
    lambda,
    driver_half: lambda * 0.2570,
    driver_full: lambda * 0.5140,
    reflector:   lambda * 0.4849,
    tip_spacer:  lambda * 0.0288,
    radius:      lambda * 0.1635,   // Horizontaler Eyelet-Radius (Top-View)
  }
}

function spreizerEmpfehlung(maxRadiusM) {
  // Pole-Bogenlänge ≈ π/2 × Radius (Halbkreis-Schüsselform).
  const poleM = maxRadiusM * Math.PI / 2
  if (poleM <= 5.5) return '5,4 m konische Glasfaserstäbe (Spiderbeam-Lieferant) — passend für 20m+'
  if (poleM <= 6.5) return '6 m Glasfaserstäbe (Verstärkung am Knick empfohlen)'
  if (poleM <= 8.0) return '7,8 m konische Stäbe (z. B. 40m-Hexbeam-Set)'
...
const topGeom = computed(() => {
  ...
  const scale = radius / maxRadius.value
...
// Seitenansicht (Schüsselform analog G3TXQ/WiMo HEX6B Side-View)
const SIDE_W = 600, SIDE_H = 320
const sideGeom = computed(() => {
  if (aktiveBaender.value.length === 0 || maxRadius.value === 0) return null
  ...
  const wires = sorted.map(band => {
    const f = band.radius / maxRadius.value
...
<template>
  ...
  <div class="card" style="background:rgba(34,197,94,0.08);border-color:#22c55e">
    <div style="display:flex;gap:10px;align-items:flex-start">
      <span style="color:#22c55e;font-size:20px;line-height:1">✓</span>
      <div>
        <div style="font-weight:600;color:var(--ts);margin-bottom:4px">G3TXQ Broadband Hexbeam</div>
        <div class="small" style="color:var(--ts);line-height:1.5">
          Werte und Geometrie nach Steve Hunt G3TXQ — referenziert über die lizenzierte WiMo EAntenna HEX6B
          Bauanleitung. 20m-Referenz: ½ Driver 214″ · Reflector 404″ · Tip Spacer 24″ · Spreader-Radius ≈ 11′4″.
          Andere Bänder linear mit λ skaliert.
        </div>
        <div class="small" style="color:var(--tm);font-style:italic;margin-top:4px">Danke an Markus, HB9EIZ, für den Hinweis und die Bauanleitung.</div>
      </div>
    </div>
  </div>
  ...
  <template v-if="aktiveBaender.length > 0">
    <div class="card">
      <h2>Maße pro Band (Drahtlängen)</h2>
      <table class="tbl hex-tbl">
        <thead>
          <tr><th>Band</th><th>Frequenz</th><th>½ Driver</th><th>Driver ges.</th><th>Reflektor</th><th>Tip Spacer</th></tr>
        </thead>
        <tbody>
          <tr v-for="b in aktiveBaender" :key="b.id">
            <td class="fw7" :style="{ color: bandColors[b.id] }">{{ b.name }}</td>
            <td class="mono">{{ fmt(b.fMHz) }} MHz</td>
            <td class="mono fw7">{{ fmt(b.driver_half) }} m</td>
            <td class="mono">{{ fmt(b.driver_full) }} m</td>
            <td class="mono">{{ fmt(b.reflector) }} m</td>
            <td class="mono">{{ fmt(b.tip_spacer) }} m</td>
          </tr>
        </tbody>
      </table>
      <p style="font-size:11px;opacity:0.65;margin:8px 0 0">
        Drahtlängen sind 3D entlang dem Spreader-Bogen (so wie das Wire durch die Eyelets läuft).
        Driver pro Band = 2× ½ Driver; Reflector ist eine durchgehende Schleife durch die 4 hinteren
        Spreader-Tips; Tip Spacer ist der PVC-Isolator zwischen Driver-Tip und Reflector-Ende.
      </p>
    </div>
    ...
    <div class="card">
      ...
      <div class="skz-bg">
        <svg v-if="topGeom" :viewBox="`0 0 ${TOP_W} ${TOP_H}`" preserveAspectRatio="xMidYMid meet">
          <!-- 6 Spreader-Achsen bis maxRadius -->
    ...
    <div class="card">
      ...
      <div class="rr hi"><span class="lbl">Spreader-Radius (horiz.)</span><span class="val">{{ fmt(maxRadius) }} m × 6 Stück</span></div>
      <div class="rr"><span class="lbl">Pole-Länge (Bogen)</span><span class="val">ca. {{ fmt(maxRadius * Math.PI / 2) }} m (≈ π/2 × Radius, Schüsselform)</span></div>
      <div class="rr"><span class="lbl">Material-Empfehlung</span><span class="val">{{ spreizerEmpfehlung(maxRadius) }}</span></div>
      <div class="rr"><span class="lbl">Hexagon-Durchmesser</span><span class="val">{{ fmt(maxRadius * 2) }} m (horizontal)</span></div>
      <template v-if="referenzBand">
        <div class="rr"><span class="lbl">½ Driver {{ referenzBand.name }}</span><span class="val">{{ fmt(referenzBand.driver_half) }} m (Drahtlänge)</span></div>
        <div class="rr"><span class="lbl">Reflektor {{ referenzBand.name }}</span><span class="val">{{ fmt(referenzBand.reflector) }} m (Drahtlänge)</span></div>
        <div class="rr"><span class="lbl">Tip Spacer {{ referenzBand.name }}</span><span class="val">{{ fmt(referenzBand.tip_spacer) }} m (PVC-Isolator)</span></div>
      </template>
