...
- [~] **N:** **Logbuch-Modul** — Phase 1 + 2 + 3 + 4b fertig (v1.5.0, 2026-05-11). Siehe `LOGBUCH_PLAN.md` für den vollen Plan.
  ...
  - **Offen:**
    ...
    - [~] **POTA-Upload** (pota.app/user/api/upload) — Activator-Logs end-to-end aus der App hochladen.
      ...
      - [x] **Schritt 2: API-Verifikation** (2026-05-16) — Endpoint `POST https://api.pota.app/adif`, multipart/form-data mit Field `file` + Filename `POTA@<REF>.adi`, 18-Byte-JSON-Quittung mit `jobId`. Asynchrones Job-System via `GET /user/jobs` (status 7=queued, 2=processed). HAR-Mitschnitt auf `~/Desktop/pota-upload.har`.
      - [~] **Schritt 3: Service-Layer** — *3a fertig (Commit 30a6656), 3b blockiert auf SRP-Auth.*
        - [x] **3a:** PotaKeychain + CognitoAuthService + PotaUploadService + PotaJobsService gebaut. UploadServicesSettings auf Username+Passwort (Keychain) umgestellt. Sheet ruft echten Service-Stack mit Polling-Loop bis Status==2.
        - [ ] **3b: SRP-Auth-Flow** — POTAs Cognito-Pool hat USER_PASSWORD_AUTH disabled (`InvalidParameterException: USER_PASSWORD_AUTH flow not enabled for this client`). Pflicht: USER_SRP_AUTH. Plan: `attaswift/BigInt` als SPM-Dep + ~250 LOC SRP6a-Helper in CognitoAuthService. Siehe Memory `project_next_session.md` für konkrete Schritte.
