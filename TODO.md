...
### Geplant
- [x] **N:** **Multi-Cluster** (2026-05-18, v1.8.13) — Pool aus mehreren DX-Cluster-Nodes parallel. `ClusterNode` (id/name/host/port/isActive, Codable-Migration vom alten `autoConnect`-Flag), `ClusterSettingsStore` mit `maxActiveNodes` (Default 3). Pool-Architektur in 3 Schritten + Polish gebaut: (1) Datenmodell + Settings-UI (afe89bf), (2) parallele Clients + Mini-Dedup über `freq+call+~time` (55d3042), (3) `alsoSeenBy`-Set pro Spot + »+N«-Confidence-Badge in der Spot-Liste (d49ce53). Polish-Release 1.8.13: Live-Status-Pille pro Node-Zeile im Settings-Tab, »N/M verbunden«-Pool-Counter in allen Top-Bars, Host/Port-Edits aktiver Nodes greifen live, ATNO-Pille als eigene Tabellenspalte (c5c735d).
- [x] **N:** **Weitere UDP-Port-Konfiguration** (2026-05-19) — UDPBridges-Modul mit Liste mehrerer Listener parallel. Zwei Protokoll-Adapter: (1) `WsjtxAdapter` deckt WSJT-X, JTDX, JS8Call, MSHV ab (alle reden NetworkMessage-Binär-Protokoll, nur Default-Ports unterscheiden sich). (2) `N1MMAdapter` parsed `<contactinfo>`- + `<spot>`-XML-Datagramme von N1MM Logger+ — QSOs landen im aktiven Log, Spots im DX-Cluster-Stream via `DXClusterViewModel.injectExternalSpot(...)`. Generischer `UDPListener` als Transport-Layer, `UDPBridgesService` als Manager (Lifecycle bei Settings-Änderung live). UI: `UDPBridgesView` mit Add/Remove + Inline-Edit. Default-Templates: WSJT-X 2237 (mit Migration aus alten WsjtxBridge-Settings), JS8Call 2242, MSHV 2333, N1MM 12060. **Offen:** fldigi (nutzt XML-RPC über TCP, kein UDP — eigenes TCP-Server-Projekt).
...
- [~] **N:** **Logbuch-Modul** — Stand 2026-05-19: Phasen 1+2+3+4b+4c+4d+4e+4f+5 sowie QSL-Tab, Stats-Dashboard, WSJT-X/JS8Call/MSHV-N1MM-UDP-Bridges, QRZ-Logbook + ClubLog + eQSL-Auto-Upload, Mode-aus-CAT durch. **Offen:** Phase 4 Contest-Engine-Polish (F1–F8-Macros), Phase 6 LoTW + Auto-POTA-Upload + WWFF/BOTA-Upload, Schedules-Tab, i18n-Review. Siehe `LOGBUCH_PLAN.md` für Phase-Historie.
  ...
  - **Offen:**
    ...
    - [~] **Upload zu QRZ Logbook / LoTW / eQSL** — Phase 6 läuft.
      - [x] **QRZ Logbook Live-Upload** (2026-05-16) — `QRZLogbookService` (POST logbook.qrz.com/api, KEY+ACTION=INSERT+ADIF), Settings unter Lookup&Upload→QRZ.com → „Logbook (Live-Upload)" mit API-Key + Auto-Toggle. Auto-Upload greift nur bei Standard-Logs (DX); Outdoor-Programme bleiben außen. Manueller Bulk-Upload via QSOTableView Rechtsklick „N QSOs an QRZ Logbook hochladen". Schema v10 mit `qrzLogbookStatus`-Spalte (0=nicht versucht, 1=OK, 2=duplicate, 3=fail), Status-Spalte „QRZ-LB" (default-hidden) zeigt ✓/⚠/⬆, Klick → retry/single-upload. Bulk parallel mit max 6 concurrent.
      - [x] **QRZ Logbook Confirmation-Sync** (2026-05-16) — `QSLTab`-Button „QRZ-Bestätigungen abrufen". `QRZLogbookService.fetchAll(apiKey:afterLogId:)` paginiert mit MAX:250 + AFTERLOGID-Inkrement (MAX:5000 lieferte still 0 Bytes — Server-Limit). Quirks gelöst: (a) GET statt POST (QRZ-Doku falsch), (b) OPTION=ALL,TYPE:ADIF zwingend (sonst 0 Bytes), (c) Browser-User-Agent + Referer + Cache-Bypass (URLSession-Default-UA wurde stillschweigend gefiltered). `LogbookManager.fetchQRZConfirmations()` matcht via Call|YYYYMMDDHHMM|Band|Mode und merged LoTW/eQSL/Direkt-QSL-Flags additiv (nie über lokal-true). Pagination-Loop bis 200 Iterationen (=50k QSOs). Verifiziert mit 7405-QSO-Log am 2026-05-16.
      - [x] **Club Log Live-Upload** — `ClubLogService` (realtime.php Form-POST + putlogs.php Bulk, App-API-Key obfuskiert in `BuildInfo.clubLogApiKey`), Auto-Upload nach addQSO greift nur in Standard-Logs, bei Auth-Fail wird Auto-Toggle automatisch pausiert (Club Log firewallt sonst die IP). Bulk via Kontextmenü, putlogs.php sequenziell.
      - [x] **eQSL.cc Live-Upload** (2026-05-19) — `EQSLService` (ImportADIF.cfm Form-POST mit `EQSL_USER`/`EQSL_PSWD`/`ADIFData`), Outcome-Enum mit `.accepted` / `.duplicate` / `.authFailed` / `.rejected` / `.network`, Schema v10→v11 mit `qsos.eqslStatus` (0=untried/1=ok/2=dup/3=fail) + `log_meta.usedEqslNickname` (Pro-Log-Override). Settings-Panel (Username/Password/Nickname/Auto/MarkQslSent) ersetzt den alten Placeholder. NewLogSheet hat optionales eQSL-Nickname-Feld. QSOTableView-Spalte „eQSL" + Kontextmenü „N QSOs an eQSL hochladen". Auto-Toggle pausiert bei Auth-Fail. Bulk max 3 parallel via TaskGroup.
      - [ ] **LoTW** — braucht tqsl-Subprocess-Signing (lokales Binary), TQ8-Signing-Workflow noch offen.
    - [x] **POTA Self-Spot** (Commit d49aa76) — `POTASelfSpotService` mit POST an `https://api.pota.app/spot/`, anonym (kein Cognito-SRP — die Auth-Hürde betraf nur den Logbook-Upload). Schema aus `cwhelchel/hunterlog` rekonstruiert. In POTAEntryForm verdrahtet, Frequenz wird in ganzzahligen kHz übertragen.
    ...
    - [~] **Phase 4** Contest-Engine — durch: 14 contests.json-Templates (HELVETIA, USKA-FD-SSB/CW, USKA-50MHZ, CQ-WW-CW/SSB, CQ-WPX-CW/SSB, IARU-HF, DARC-WAG, ARRL-DX-CW/SSB, WAE-DX-CW/SSB), `ContestScoringEngine` mit Live-Score, `ContestStatsPanel` (Rate-Meter 10/60min + Band×Mult-Matrix + Heatmap), Run/S&P-Toggle, DupeChecker, SpotEvaluator, Map-Tab. **SCP (2026-05-19)**: `SCPService` mit Multi-Source-DB (supercheckpartial.com ~50k + Club Log ~180k), Binary-Search-Prefix-Lookup, Bundle-Cold-Start, Auto-Stale-Popup nach 14 Tagen, Settings-Section unter »Daten« mit Toggle + manueller Update-Button pro Quelle. Suggest-Flow-Grid unter ContestEntryForm-Footer. **Offen:** F1–F8 Macros (CW/SSB-Phrasen), mehr Contest-Templates.
    ...
    - [x] **Phase 4f** BOTA-Modus komplett (2026-05-14) + WWBOTA-Polish (2026-05-17): LogType.bota, BOTARefService mit api.wwbota.org-Endpoint, Initial-CSV-Bundle (~26.7k Bunker mit Versions-Check), BOTAReference im WWBOTA-Format B/XX-NNNN inkl. Präfix-Normalisierung, NewBOTALogSheet mit Activator/Hunter + Multi-Bunker, BOTAEntryForm (1-QSO-Regel), BOTASpotsView gefiltert aus DX-Cluster mit B/-Prefix-Erkennung, BOTAMapTab mit Bunker-Pins + B2B-Indikator, Awards-Sub-Tab BOTA. ADIF-Export jetzt WWBOTA-konform (`MY_SIG=WWBOTA` + `MY_SIG_INFO`, Komma-Liste laut Guide erlaubt) + APP_HAMTOOLS-Felder für Re-Import-Roundtrip. Siehe `BOTA_PLAN.md` für Details. **Offen:** Auto-Upload zu wwbota.net (kein offizielles API).
    - [~] **Phase 6** Upload-APIs — QRZ Logbook + Confirmation-Sync erledigt (siehe oben). **Manueller** Outdoor-Upload jetzt plattform-konform (2026-05-17): POTA-ADIF mit MY_SIG=POTA + MY_SIG_INFO (keine non-standard MY_POTA_REF mehr), WWBOTA-ADIF mit MY_SIG=WWBOTA, SOTA-CSV-V2-Export für sotadata.org.uk mit eigenem Toolbar-Button (`SOTACSVExporter`, pro Summit gruppiert + zeit-sortiert, S2S in Spalte 9). Bug 3 (Callbook-Auto-Fill in Outdoor-EntryForms) auch erledigt — QTH/GRIDSQUARE/COUNTRY/CONT/CQZ/ITUZ kommen jetzt automatisch ins ADIF. **Offen:** Auto-Upload-APIs für LoTW/eQSL/Club Log/pota.app/wwff.co/wwbota.net — vom User explizit ans Sessionende geschoben.
    - [x] **Phase 3-Rest** (2026-05-16): Distance/Bearing + QRZ-Image-Cache-Persistierung komplett.
      - [x] **Distance/Bearing automatisch beim QSO-Logging** — LogbookManager.addQSO/updateQSO rechnet jetzt `distanceKm` + `bearingDeg` aus eigenem `qthLocator` (AppStorage) und QSO-Locator. Spalten in QSOTableView (Distanz / Peilung) zeigen die Werte; ADIF-Export schreibt DISTANCE + ANT_AZ. Plus Bulk-Backfill via Spalten-Menü → „Wartung → Distanz/Peilung für alle QSOs nachrechnen…" mit Ergebnis-Alert.
      - [x] **QRZ-Image-Cache-Persistierung** — `QRZImageCache` (Singleton) speichert Profilbilder in `<DataRoot>/Cache/qrz-images/<sha256-prefix>.bin` mit 30-Tage-TTL. Beim ersten Cache-Zugriff pro Session läuft ein Background-Sweep über abgelaufene Files. `CachedQRZImage`-View ersetzt `AsyncImage` in QSOEntryPanel — Second-Open zeigt Bild ohne Spinner.
    - [~] **i18n** (2026-05-19) — Infrastruktur + DE/EN-Erstbestand komplett. Package.swift hat `defaultLocalization: "de"`, `Sources/HAMRechner/Content/Localizable.xcstrings` als Resource. `tools/extract_strings.py` scannt alle Swift-Files via Regex (läuft vor jedem `swift build` via `run.sh`). **866 Source-Keys** erfasst, **alle 866 EN-Übersetzungen** vorhanden: 52 manuell geseedet (`state="translated"`, Top-Menü/Tabs/QSO-Form/Buttons), 814 via DeepL-API mit Token-Schutz für Swift-Interpolation `\(…)` (`state="needs_review"`, ~30k Zeichen, weit unter Free-Tier-Limit). Architektur ist n-sprachig — weitere Locales nur xcstrings-Spalte. **Offen:** Review der DeepL-Übersetzungen (besonders Funk-Fachvokabular wie "1.FZ" → "1st half-year"), Strings aus Enum-DisplayNames separat migrieren. Siehe Memory `project_i18n.md`.
...
- [x] **N:** **Bandplan als eigenes Fenster statt Logbuch-Sub-Tab** (2026-05-17) — Bandplan-Ansicht raus aus dem Logbuch-Tab-Bar (zu wenig Platz nach unten), jetzt Single-Instance-Popup über Fenster-Menü „Bandplan-Fenster" (⌘⇧P) mit Default 980×760. Registriert als eigene `WindowGroup(id: "bandplan")` in HAMRechnerApp.
