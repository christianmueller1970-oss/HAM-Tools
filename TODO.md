...
- [x] **NW:** **Hexbeam-Rechner G3TXQ-konform überarbeitet** — Quelle: WiMo EAntenna HEX6B Bauanleitung Rev. V2.1 (offizielle G3TXQ-Lizenz). Faktoren aus 20m-Referenzmaßen abgeleitet, linear mit λ skaliert: ½ Driver 0,2570·λ · Reflector 0,4849·λ · Tip Spacer 0,0288·λ · Spreader-Radius (horiz.) 0,1635·λ. Erklärung Schüsselform: Pole-Bogenlänge ≈ π/2 × Horizontal-Radius (≈ Halbkreis). Native + Web: Tabelle hat jetzt ½ Driver / Driver gesamt / Reflektor / Tip Spacer (Spreader wandert in Summary). Draufsicht: Reflector korrigiert auf 3-Sehnen-Polygon durch hintere 4 Spreader-Tips (90°→150°→210°→270°), Tip-Spacer-Marker (gefüllter Driver-Endpunkt + offener Reflector-Anfang) am Front-Spreader-Tip. Warning-Banner ersetzt durch grünen G3TXQ-Quellen-Hinweis. hexbeam.md mit Bracket-Anordnung (12E/16E/20E pro Band) und HEX6B-Specs erweitert.
...
- [~] **W:** **Antennen-Simulator (NEC2 via WASM)** — **Phase 1+2+3 erledigt 2026-05-10:** Phase 1 PoC mit nec2c.wasm + Worker. **Phase 2: Mehrelement-Editor** — Drahtmodell-Tabelle (Tag/Segs/X1/Y1/Z1/X2/Y2/Z2/Radius pro Wire, add/remove), Templates (Dipol, Inverted-V, 2-El Yagi, 3-El Yagi, Quad-Loop), Excitation-Picker (Radio + Segment), 2D-Modellvorschau (Top-Down + Side per SVG, Speisedraht rot hervorgehoben). Live auf toolbox.funkwelt.net/#/antennensim. **Phase 3: Frequenz-Sweep** — Multi-Frequenz-Worker mit FR-Steps, Output-Parser sammelt alle Blöcke, Vue zeigt SWR-Linien-Chart und R+jX-Chart über Frequenz, automatische Resonanz-Erkennung (X=0 linear interpoliert) und Bandbreiten-Auswertung (SWR ≤ 2 Bereich). Pattern wird im Sweep-Modus übersprungen (zu groß). UI: Toggle "Frequenz-Sweep aktivieren" + f_start/f_stop/Schritte. Spinner-Buttons in Tabelle weggemacht (waren zu klein). **Phase 5 (Native-Integration) erledigt:** WKWebViewWrapper als NSViewRepresentable, lädt toolbox.funkwelt.net/#/antennensim, Loading-Indicator, Error-Fallback mit Retry. Calculator-Enum + Sidebar-Eintrag in "Signale & Tools" mit Antennen-Icon. **Phase 4 offen (optional):** 3D-Pattern (Three.js Mesh), Templates direkt aus den anderen Antennen-Rechnern importieren ("Im Sim öffnen"-Button bei Hexbeam/Yagi).
