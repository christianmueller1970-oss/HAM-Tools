...
- [~] **N:** **Logbuch-Modul** — Phase 1 + 2 + 3 + 4b fertig (v1.5.0, 2026-05-11). Siehe `LOGBUCH_PLAN.md` für den vollen Plan.
  ...
  - **Offen:**
    ...
    - [~] **i18n** Infrastruktur (2026-05-19) — Package.swift hat `defaultLocalization: "de"`, `Sources/HAMRechner/Content/Localizable.xcstrings` als Resource. `tools/extract_strings.py` scannt alle Swift-Files via Regex und erfasst Source-Keys aus den üblichen SwiftUI-Patterns (Text/Label/Button/Toggle/Picker/TextField/Section/GroupBox/.help/.alert/.tabItem etc.) — läuft vor jedem `swift build` via `run.sh`. **866 Source-Keys** beim ersten Lauf erfasst. **~52 EN-Übersetzungen** initial via `tools/seed_en_translations.py` (Top-Menü, Tab-Labels, QSO-Form, häufige Buttons). Architektur ist n-sprachig — weitere Locales nur xcstrings-Spalte hinzufügen, kein Code-Change. **Offen:** EN-Übersetzungen für die restlichen ~814 Strings (DeepL-Skript oder manuell), Strings aus Enum-DisplayNames separat migrieren. Siehe Memory `project_i18n.md`.
