...
// Schlanke BOTA-QSO-Entry-Form. Aktiv wenn Logbuch ein BOTA-Log ist.
// Strukturparallel zu WWFFEntryForm, ohne 44-QSO-Counter — BOTA hat
// keine strikten Aktivierungsregeln (1 QSO reicht meist).
struct BOTAEntryForm: View {
    ...
    @State private var theirRefMatch: BOTAReference?
    ...
    private var myRef: BOTAReference? {
        guard let ref = currentLog?.botaRef else { return nil }
        return bota.ref(forReference: ref)
    }
    ...
    var body: some View {
        ...
        .onChange(of: logBridge.pendingBotaSpot) { _, spot in
            if let s = spot { applySpot(s); logBridge.pendingBotaSpot = nil }
        }
    ...
    private var sessionBar: some View {
        ...
        let myRefLabel: String = {
            if let multi = log?.botaRefs, !multi.isEmpty { return multi }
            return log?.botaRef ?? "—"
        }()
    ...
    private var formGrid: some View {
        Grid(alignment: .leading, horizontalSpacing: 8, verticalSpacing: 8) {
            ...
            GridRow {
                labeled("Their Bunker") {
                    VStack(alignment: .leading, spacing: 2) {
                        TextField("DE-1234 (B2B oder Hunter)", text: $theirRef)
                        ...
                        if let r = theirRefMatch {
                            ...
                            Text("Bunker unbekannt — wird trotzdem gespeichert")
                                .font(.caption2)
    ...
    private func saveQSO() {
        ...
        if !trimmedTheirs.isEmpty {
            qso.theirBotaRef = trimmedTheirs.uppercased()
        }
    ...
    private func applySpot(_ s: BOTASpot) {
    ...
    private func resolveTheirRef(_ ref: String) {
        ...
        theirRefMatch = bota.ref(forReference: trimmed)
    }
