...
| **5a** | **Read-only Foundation** | TRXProfile + trx-profiles.json, RigctldProcess (launch aus Bundle-Pfad), RigctldClient (TCP-Client), CATController (Poll-Loop nur lesen), CATSettingsView. RadioState.source flippt auf `.cat` wenn Live. | 2 Sessions | Frequenz/Mode vom Radio kommt in der App an |
