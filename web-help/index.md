---
layout: home

hero:
  name: HAM-Tools
  text: Anleitungen & Tipps
  tagline: Der macOS-Logger für DX, Contest, POTA, SOTA, WWFF und BOTA — von HB9HJI für die Funkwelt.
  image:
    src: /icon.svg
    alt: HAM-Tools
  actions:
    - theme: brand
      text: Erste Schritte
      link: /getting-started
    - theme: alt
      text: Module-Übersicht
      link: /modules/logbuch
    - theme: alt
      text: Download
      link: https://toolbox.funkwelt.net/app/dmg/latest.dmg

features:
  - icon: 📒
    title: Multi-Log
    details: Mehrere Logbücher gleichzeitig — Lebens-Log, POTA-Sessions, Contests. Wechseln per Klick, alle Daten lokal in .htlog-Dateien.
  - icon: 🏆
    title: Contest-Modus
    details: 14 Templates (HB-Helvetia, CQ-WW, CQ-WPX, IARU, ARRL-DX, WAE, DARC-WAG …) mit Wizard, Live-Score, Dupe-Markierung und Cabrillo-Export.
  - icon: 🌲
    title: POTA + SOTA + WWFF + BOTA
    details: Vier komplette Award-Programme mit lokalen Ref-Datenbanken, Activator/Hunter-Workflow, programm-spezifischen Spots-Feeds, Maps und Awards-Aggregation. Outdoor-Sub-Picker im QSO-Panel.
  - icon: 📡
    title: CAT-Anbindung
    details: 24 Radio-Modelle (Yaesu, Icom, Kenwood, Elecraft) via Hamlib. ICOM mit korrekter CI-V-Adresse pro Modell. Multi-Config zum schnellen Wechseln.
  - icon: 🌐
    title: DX-Cluster
    details: Eigener DXSpider plus POTA/SOTA-Live-Feeds und WWFF/BOTA-Filter aus dem DX-Cluster. Spots gefiltert auf Watchlist, Award-relevante Modes, Contest-Bänder. Klick füllt Log + setzt CAT.
  - icon: 🧮
    title: 25+ Rechner
    details: Antennen-Dimensionierung (Dipol, Yagi, Magnetic Loop, Hexbeam …), Balun, SWR, Smith-Chart, Linkbudget, QTH-Locator, Antennen-Simulator.
---

## Was ist HAM-Tools?

Eine native **macOS-App** für Funkamateure mit Fokus auf:
- **Logbuch** mit allen üblichen Award-Workflows (DXCC, WAZ, WAS, POTA, SOTA, WWFF, BOTA)
- **Contest-Logger** mit Cabrillo V3-Export für die wichtigsten Major-Contests
- **CAT-Anbindung** für Yaesu / Icom / Kenwood / Elecraft via Hamlib
- **Antennen-Rechner** für Drahtantennen, Richtstrahler, Spezialantennen, Spulen

Distribution per **DMG mit Apple-Notarisierung** — Gatekeeper-freundlich. Lizenzsystem
mit eingebauter **Update-Verteilung** (Cmd+Opt+U), Modell *Lifetime + 12 Monate Updates*.

## Schnellstart

::: info Voraussetzung
**macOS 14 (Sonoma) oder neuer**, Apple Silicon oder Intel. macOS 12/13 wird nicht unterstützt — siehe [FAQ](/faq#auf-welchen-macs-lauft-ham-tools).
:::

1. **DMG herunterladen** und in `/Applications` ziehen
2. **App starten** (beim ersten Öffnen ggf. Systemeinstellungen → Datenschutz)
3. **Lizenz-String** in **Einstellungen → Lizenz** einfügen (oder 50 QSO Demo testen)
4. **Eigenes Rufzeichen** in **Einstellungen → Station** eintragen
5. **Erstes QSO** im Logbuch-Tab loggen

Detaillierter: [Erste Schritte →](/getting-started)

## Hilfe bekommen

- **Bug melden:** in der App `Cmd+Shift+B` oder via Menüleiste → Hilfe → Bug melden
- **Lizenz anfragen:** `hb9hji@funkwelt.net`
- **FAQ:** [Häufige Fragen](/faq) | [Bekannte Bugs](/known-bugs)

73 de HB9HJI
